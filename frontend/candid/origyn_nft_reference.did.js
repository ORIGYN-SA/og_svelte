export const idlFactory = ({ IDL }) => {
  ...
  const Result_15 = IDL.Variant({ ok: BidResponse, err: OrigynError });
  ...
  const Result_10 = IDL.Variant({ ok: EscrowResponse, err: OrigynError });
  const Result_2 = IDL.Variant({ ok: IDL.Text, err: OrigynError });
  const StorageMetrics = IDL.Record({
    available_space: IDL.Nat,
    allocated_storage: IDL.Nat,
  });
  const Result_9 = IDL.Variant({ ok: StorageMetrics, err: OrigynError });
  ...
  const SaleStatus = IDL.Record({
    token_id: IDL.Text,
    sale_type: IDL.Variant({ auction: AuctionState }),
    sale_id: IDL.Text,
  });
  const NFTInfo = IDL.Record({
    metadata: CandyValue,
    current_sale: IDL.Opt(SaleStatus),
  });
  const Result_5 = IDL.Variant({ ok: NFTInfo, err: OrigynError });
  ...
  const Nft_Canister = IDL.Service({
    ...
    bid_nft_origyn: IDL.Func([BidRequest], [Result_15], []),
    ...
    end_sale_nft_origyn: IDL.Func([IDL.Text], [Result_11], []),
    escrow_nft_origyn: IDL.Func([EscrowRequest], [Result_10], []),
    ...
    get_canister_network_nft_origynt: IDL.Func(
      [],
      [IDL.Opt(Principal)],
      ["query"]
    ),
    ...
    get_storage_metrics: IDL.Func([], [Result_9], ["query"]),
    ...
    manage_storage_nft_origyn: IDL.Func([ManageStorageRequest], [Result_7], []),
